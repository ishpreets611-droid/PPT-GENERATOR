...
def generate_image(img_prompt, slide_no = 1):
  """This function helps funciton to generate
  ...
  img = Image.open(f"ai_image{slide_no}.jpeg")
  return url
...
def agent_prompt(query):
  ...
  you are aprofessional ppt designer,
  based on user given query, your task is to professional
  HTML output with no markdowns.
  user query: {query}"""
...
if (user_input) and (leader_agent):
  # TAB 1 code
  with tab1:
    if st.button("Generate Image", key = "Gen-Iamge"):
      with st.spinner("Running Agent"):
        try:
          img = generate_image(user_input)
          st.image(img)
        except:
          url = f"https://image.pollinations.ai/{user_prompt}"
          time.sleep(4)
          st.image(url)
